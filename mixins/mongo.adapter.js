...
class MongoDbAdapter {
  ...
  connect() {
    ...
    return this.client.connect().then(() => {
      ...
      this.collectionRevisions = this.db.collection(
        `${this.service.schema.collection}Revisions`
      );
  ...
  updateById(_id, update) {
    const itemByUpdate = {$set: update};
    return this.collection
      .findOneAndUpdate({_id: this.stringToObjectID(_id)}, itemByUpdate, {returnOriginal: false})
      .then((res) => res.value);
  }
  ...
  /**
   * Transforms MongoDB's '_id' into users defined 'idField'
   * @param {Object} entity
   * @param {String} idField
   * @memberof MongoDbAdapter
   * @returns {Object} Modified entity
