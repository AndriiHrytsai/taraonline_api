    ...
    methods: {
      ...
      socialAuthCallback (setting, providerName) {
        return (req, res) => (err) => {
          ...
          if (mixinOptions.cookies) {
            res.setHeader('Set-Cookie', cookie.serialize(mixinOptions.authCookieName || 'authToken', req.user.token, Object.assign({
              path: '/',
              maxAge: process.env.JWT_SOCIAL_AUTH_TOKEN_MAX_AGE
            })));

            res.setHeader('Set-Cookie', cookie.serialize(mixinOptions.userDataCookieName || 'userData', req.user, Object.assign({
              path: '/',
              maxAge: process.env.JWT_SOCIAL_AUTH_TOKEN_MAX_AGE
            })));
          }
      ...
      socialPhoneAuthCallback (setting, providerName) {
        return (req, res) => (err) => {
          if (err) {
            this.logger.warn('Authentication error.', err);
            this.sendError(req, res, err);
            // res.writeHead(301, {Location: process.env.LOGIN_PAGE});
            // res.end();
            return;
          }
